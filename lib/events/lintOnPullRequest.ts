...
type LintStep = Step<
	EventContext<LintOnPullRequestSubscription, LintConfiguration>,
	LintParameters
>;
...
const SetupStep: LintStep = {
	...
	run: async (ctx, params) => {
		...
		await ctx.audit.log(
			`Starting commitlint on ${repo.owner}/${repo.name}`,
		);
...
const NpmInstallStep: LintStep = {
	...
	run: async (ctx, params) => {
		...
		if (
			!(await fs.pathExists(
				params.project.path("node_modules", ".bin", "commitlint"),
			))
		) {
			if (
				!modules.includes("commitlint") &&
				!modules.includes("@commitlint/cli")
			) {
...
const RunCommitlintStep: LintStep = {
	...
	run: async (ctx, params) => {
		...
		const configs = await project.globFiles(params.project, [
		...
		if (configs.length === 0 && !pj.commitlint && !!cfg.config) {
			await fs.writeFile(
				params.project.path(".commitlintrc.json"),
				cfg.config,
			);
		}
		...
		await ctx.audit.log(
			`Running commitlint with: $ commitlint ${argsString}`,
		);
		...
		for (const commit of commits) {
			...
			results.push(
				await params.project.spawn(
					...
					[
						"-c",
						`echo "${commit.message}" | ${cmd} ${args.join(" ")}`,
					],
					{
